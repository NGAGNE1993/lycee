...
import com.mycompany.myapp.domain.ComptableMatiere;
import com.mycompany.myapp.repository.ComptableMatiereRepository;
import com.mycompany.myapp.service.ComptableMatiereService;
...
class ComptableMatiereResourceIT {
    ...
    @Mock
    private ComptableMatiereRepository comptableMatiereRepositoryMock;

    @Mock
    private ComptableMatiereService comptableMatiereServiceMock;
    ...
    void getAllComptableMatieresWithEagerRelationshipsIsEnabled() throws Exception {
        when(comptableMatiereServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restComptableMatiereMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(comptableMatiereServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllComptableMatieresWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(comptableMatiereServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
