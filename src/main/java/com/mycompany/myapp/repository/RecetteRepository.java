...
public interface RecetteRepository extends JpaRepository<Recette, Long> {
    ...
    @Query(
        value = "select distinct recette from Recette recette left join fetch recette.lyceesTechniques left join fetch recette.comptableFinancier",
        countQuery = "select count(distinct recette) from Recette recette"
    )
    Page<Recette> findAllWithToOneRelationships(Pageable pageable);

    @Query(
        "select distinct recette from Recette recette left join fetch recette.lyceesTechniques left join fetch recette.comptableFinancier"
    )
    List<Recette> findAllWithToOneRelationships();

    @Query(
        "select recette from Recette recette left join fetch recette.lyceesTechniques left join fetch recette.comptableFinancier where recette.id =:id"
    )
