...
class SerieResourceIT {
    ...
    private static final Series DEFAULT_NOM_SERIE = Series.S1;
    private static final Series UPDATED_NOM_SERIE = Series.S2;
    ...
    void getAllSeriesWithEagerRelationshipsIsEnabled() throws Exception {
        when(serieServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSerieMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(serieServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllSeriesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(serieServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
